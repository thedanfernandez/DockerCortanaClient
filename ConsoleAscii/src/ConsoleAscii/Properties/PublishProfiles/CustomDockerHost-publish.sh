...
# Prase publish settings xml file
...
sed -e "s,{{ProjectName}},$projectName,g" -e "s,{{DockerPublishContainerPort}},$dockerPublishContainerPort,g" -i.bak "$dockerfilePath"
...
if [ "$dockerBuildOnly" == "False" ]; then
    ...
    echo "Docker command: docker $dockerAuthOptions -H $dockerServerUrl run -t -d$publishPort $dockerImageName"
    containerId=$(docker "$dockerAuthOptions" -H "$dockerServerUrl" run -t -d$publishPort "$dockerImageName")
